...
注意单次触发的时刻抖动仍取决于主循环的空闲程度（协作式内核实然）。
...
| 想监控丢了多少事件 | — | 读 `mote_dropped_count()`：因队列满/事件无效被**实际丢弃**的累计数（单次 RETRY 定时器的满队重试不计入——它最终会送达）。想知道"丢的是哪个事件"，用 `mote_set_drop_hook()` 注册回调（注意：钩子在关中断上下文运行，只允许事件/邮箱 API，禁止定时器/任务 API） |
