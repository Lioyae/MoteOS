 ...
 *    把整拍入账一次（SysTick_Handler 与 mote_idle 互斥消费，杜绝
...
#elif defined(MOTE_PORT_CH32)
/* WCH 青稞 SysTick：64 位比较寄存器、向上计数（ch32v00x.h 提供寄存器）。
 * 单次 nap 上限取 31 位（回绕比较数学留裕量） */
#define MOTE_SYST_MAX_CYCLES 0x7FFFFFFFu
#else
#error "MOTE_TICKLESS requires a platform port tag (MOTE_PORT_CORTEXM or MOTE_PORT_CH32)"
#endif

/* 单拍周期数（编译期常量；MOTE_PORT_HCLK_HZ≥1000 已由配置校验）。
 * HCLK 非 1000 整数倍时取整导致 nap 最多早醒 <1ms——只早不迟，
 * 安全性优先于精度（早醒后 mote_idle 会按锚点差额重新入账） */
#define MOTE_CYCLES_PER_MS (MOTE_PORT_HCLK_HZ / 1000u)

/* 单次 nap 上限（ms）：受 SysTick 计数器位宽与 2^31 回绕数学共同约束。
 * 全为编译期常量——idle 路径不再需要运行时 64 位除法/乘法 */
#define MOTE_NAP_CAP_MS ((MOTE_SYST_MAX_CYCLES) / (MOTE_CYCLES_PER_MS))
#if MOTE_NAP_CAP_MS > 0x40000000u
#define MOTE_MAX_NAP_MS_CONST 0x40000000u
#elif MOTE_NAP_CAP_MS < 1u
#define MOTE_MAX_NAP_MS_CONST 1u
#else
#define MOTE_MAX_NAP_MS_CONST MOTE_NAP_CAP_MS
#endif
...
{
    uint32_t ms = s_acc_cycles / MOTE_CYCLES_PER_MS;

    if (ms != 0u) {
        s_acc_cycles -= ms * MOTE_CYCLES_PER_MS;
    }
    return ms;
}

/* 时基入账（ISR 与 mote_idle ① 共用，调用方处于临界区内）：
 ...
 * 负溢出。 */
static void mote_systick_account(void)
{
    bool wrapped = mote_systick_matched();
    uint32_t rem = mote_systick_remaining();
    uint32_t elapsed;
 ...
 * 32 位无溢出依据：ms 被钳制在 MOTE_MAX_NAP_MS_CONST 内 →
 * ms*MOTE_CYCLES_PER_MS ≤ MOTE_SYST_MAX_CYCLES。
...
static void mote_systick_set(uint32_t ms)
{
    ...
    if (ms > MOTE_MAX_NAP_MS_CONST) {
        ms = MOTE_MAX_NAP_MS_CONST;
    }
    cycles = ms * MOTE_CYCLES_PER_MS;
...
void mote_port_systick_handler(void)
{
    ...
    /* 若拍界标志已被 mote_idle 抢先消费（中断被关期间匹配），
     * account 只入账部分拍；否则整拍在此入账。二者互斥不重复 */
    mote_systick_account();
...
void mote_idle(uint32_t next_due)
{
    ...
    /* ② 按下一 deadline 计算 nap */
    nap = MOTE_MAX_NAP_MS_CONST; /* 无到期项：长睡等任意中断 */
