 ...
 * 编译必须带：-DMOTE_TICKLESS=1 -DMOTE_PORT_HCLK_HZ=24000u
 *
 * 在固定拍冒烟之上，覆盖 tickless 的空闲路径（固定拍冒烟不编译此分支）：
 *   1. mote_sleep → mote_idle 的入账/长拍编程/重装全流程实际执行
 ...
 * 宿主指令，冒烟无法在 CI 内完成。因此把换算常量 MOTE_PORT_HCLK_HZ
...
#define SYST_RELOAD (MOTE_PORT_HCLK_HZ / 1000u)
...
int main(void)
{
    ...
    mote_timer_start(&blink_timer, 0, NULL, 1000, true);

    /* 先配定时器再开 tick：due = 1000 */
    systick_start();

    /* 与 mote_loop 等价（带守卫）：无事件时进 mote_sleep，
     * 即 tickless 长拍 + wfi。QEMU TCG 下宿主速度与虚拟时间脱钩，
     * 守卫用 mote_ticks() 计虚拟时间 */
    while (s_blinks < 3 && mote_ticks() < 8000u) {
        if (!mote_poll()) {
            mote_sleep();
        }
    }
    ...
    if (s_blinks < 3 || mote_dropped_count() != 0 ||
        mote_ticks() < 3000u || mote_ticks() > 4000u) {
