...
- ✅ 已验证：宿主机单元测试/交错测试（含 ASan/UBSan、多随机种子）、断言开启构建、最坏配置构建（队列 255）、M0+/M3/RV32 交叉编译与体积断言、真实 SDK 例程编译——全部由 CI 自动化
...
| 项 | 占用 |
|---|---|
| 内核 Flash | RV32EC 约 2.0KB、Cortex-M0+ 约 1.2KB（CI 交叉编译 -Os 实测三个内核 .o，断言 <2.5KB） |
