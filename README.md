...
| 项 | 占用（实测，-Os） |
|---|---|
| 内核 Flash | RV32EC 约 2.0KB（2004B），Cortex-M0+ 约 1.2KB（1236B），CI 断言 <2.5KB |
| 内核 RAM | 约 280B（事件队列 16 槽 + 延时槽 4 + 任务槽 4），CI 断言 <512B |
| 完整点灯例程 | CH32V003 全工程实测：FLASH 2.7KB / RAM 712B（含启动文件与栈） |
...
| 定时器 | 静态定义；32 位回绕安全；满队策略可选：重试（默认，至少一次）/ 丢弃（严格截止）/ 最新（replace 语义） |
