...
- ✅ Board-tested: CH32V003 (MounRiver + WCH EVT SDK) passed Stage1-15: fixed tick, queue pressure, replace, delayed posts, mailbox, task API, timer control, drop hook, `mote_loop/mote_sleep/mote_next_due`, tickless accounting, WFI wakeup, and an integrated long run. Stage15 tickless + WCH `__WFI()` reached 671000 ticks with `drop=0 err=0`
- ❌ Not verified: real hardware other than CH32V003. Interrupt latency, worst-case critical-section duration, and actual low-power current still need target-board measurements; CH32V003 WFI wakeup is verified, but current depends on board LEDs, UART, regulators, debugger, and external loads
...
| Full blink example | CH32V003 simple example measured manually: FLASH 2.7KB / RAM 712B; Stage15 integrated tickless/WFI stress project: FLASH 9628B / RAM 880B (including startup and stack; **example size is not CI-asserted**). On 2KB-RAM chips, app data and stack budget remains tight |
