...
**Development preview (v0.x), first board verification completed on CH32V003.**
...
- ✅ Board-tested: CH32V003 (MounRiver + WCH EVT SDK) passed Stage1-16: fixed tick, queue pressure, replace, delayed posts, mailbox, task API, timer control, drop hook, `mote_loop/mote_sleep/mote_next_due`, tickless accounting, WFI wakeup, integrated long run, and a dedicated low-power measurement example. Stage15 tickless + WCH `__WFI()` reached 671000 ticks with `drop=0 err=0`
- ✅ Measured: Stage16 disables USART after the boot banner and emits optional 2s PC1/PC0 diagnostic pulses. 10Ω and 5Ω shunt readings agree: board-level low plateau ~9.4mA, LED-on plateau ~10.6mA, wake/activity peak ~12.3mA. This includes board/external loads and is not bare-chip sleep current
- ❌ Not verified: real hardware other than CH32V003. Interrupt latency, worst-case critical-section duration, and chip-level low-power current still need target-board measurements; CH32V003 WFI wakeup is verified, but current depends on board LEDs, UART, regulators, debugger, and external loads
...
> CH32V003 has first-pass board verification; other chips are still covered only by cross-compilation or QEMU.
...
| Full blink example | CH32V003 simple example measured manually: FLASH 2.7KB / RAM 712B; Stage15 integrated tickless/WFI stress project: FLASH 9628B / RAM 880B; Stage16 low-power measurement project: FLASH 3212B / RAM 396B (including startup and stack; **example size is not CI-asserted**). On 2KB-RAM chips, app data and stack budget remains tight |
...
| Low power | Deadline-aware: `mote_next_due()` exposes the next expiry; the kernel sleeps (into `mote_idle(next_due)`) only when the queue is empty and nothing is due; optional **tickless** idle (`MOTE_TICKLESS=1`) reloads SysTick to the next deadline before wfi and restores the fixed rate on wake. CH32V003 verifies WCH `__WFI()` wakes from the MoteOS critical-section path via SysTick; Stage16 provides a boot-print-only current measurement example; other chips still need board-level confirmation |
| Critical section | Save/restore style (PRIMASK / mstatus), nesting-safe |
...
>
> The CH32 port owns `SysTick_Handler` by default with a strong symbol. If your
> project provides its own `SysTick_Handler`, define
> `MOTE_PORT_DEFAULT_SYSTICK_HANDLER=0` project-wide and call
> `mote_port_systick_handler()` from your handler; do not call `mote_tick()`
> directly in tickless builds.
